...
def get_pile(nsamples, seed, seqlen, model):
    ...
    traindata = load_dataset(
        "json",
        data_files=
        '/cpfs01/user/chenmengzhao/prompt_quantization/val.jsonl.zst',
        split="train")

    tokenizer = AutoTokenizer.from_pretrained(model, use_fast=False)
    trainenc = tokenizer("\n\n".join(traindata['text'][:1000]),
                         return_tensors='pt')
...
def get_loaders(
    ...
    if 'custom' in name:
        return get_custom(
            ...
            seqlen,
            model,
            calib_data='/root/workspace/external_data/llama_calib.jsonl')
    ...
    if 'mix' in name:
        wiki_train, wiki_val = get_wikitext2(nsamples // 3, seed, seqlen,
                                             model)
