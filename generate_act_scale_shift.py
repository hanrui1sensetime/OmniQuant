...
def get_act_shifts(model, dataloader, num_samples=128):
    ...
    def stat_tensor(name, tensor):
        ...
        if name in act_shifts:
            act_shifts[name] = 0.99 * act_shifts[name] + 0.01 * (
...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=str,
        default=
        '/cpfs01/user/chenmengzhao/llama_quantization/llama-hf/llama-7b',
        help='model name')
    parser.add_argument('--scales-output-path',
                        type=str,
                        default='./act_scales/',
                        help='where to save the act scales')
    parser.add_argument('--shifts-output-path',
                        type=str,
                        default='./act_shifts/',
